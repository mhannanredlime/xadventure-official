  ...
  <div class="container">
      ...
      <div class="col-12">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
          <div class="flex-grow-1">
            <h1 class="fw-bold jatio-color mb-2" style="font-size: 2.5rem;">Welcome back, {{ $customer->name }}!</h1>
    ...
    <div class="row mb-5">
      <div class="col-xl-3 col-md-6 mb-4">
        <div class="card border-0 shadow-sm h-100" style="border-radius: 15px;">
          <div class="card-body text-center p-4">
            <div class="d-inline-flex align-items-center justify-content-center jatio-bg-color bg-opacity-10 rounded-circle mb-3" style="width: 70px; height: 70px;">
      ...
      <div class="col-xl-3 col-md-6 mb-4">
        <div class="card border-0 shadow-sm h-100" style="border-radius: 15px;">
          <div class="card-body text-center p-4">
            ...
            <h3 class="fw-bold text-success mb-2" style="font-size: 2.2rem;">{{ $completedReservations }}</h3>
      ...
      <div class="col-xl-3 col-md-6 mb-4">
        <div class="card border-0 shadow-sm h-100" style="border-radius: 15px;">
          <div class="card-body text-center p-4">
            <div class="d-inline-flex align-items-center justify-content-center bg-warning bg-opacity-10 rounded-circle mb-3" style="width: 70px; height: 70px;">
      ...
      <div class="col-xl-3 col-md-6 mb-4">
        <div class="card border-0 shadow-sm h-100" style="border-radius: 15px;">
          <div class="card-body text-center p-4">
            <div class="d-inline-flex align-items-center justify-content-center bg-info bg-opacity-10 rounded-circle mb-3" style="width: 70px; height: 70px;">
          ...
          <div class="card-body p-0">
                  ...
                  <tbody>
                    @foreach($groupedUpcomingReservations as $group)
                      ...
                      <tr class="{{ $isMultiPackage ? 'table-info' : '' }}">
                        <td class="px-4 py-3">
                          <div>
                            @if($isMultiPackage)
                              ...
                              <br>
                              <small class="text-muted">{{ $reservations->count() }} packages</small>
                            @else
                              <strong>{{ $primaryReservation->packageVariant->package->name ?? 'N/A' }}</strong>
                              <br>
                              <small class="text-muted">{{ $primaryReservation->packageVariant->variant_name ?? 'N/A' }}</small>
                            @endif
                          </div>
                        </td>
                        <td class="px-4 py-3">{{ $primaryReservation->date ? \Carbon\Carbon::parse($primaryReservation->date)->format('M d, Y') : 'N/A' }}</td>
                        <td class="px-4 py-3">{{ $primaryReservation->scheduleSlot->time_slot ?? 'N/A' }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                          <a href="{{ route('customer.reservations.details', $primaryReservation->id) }}" class="btn btn-sm btn-outline-warning px-3">
            ...
            @else
              <div class="text-center py-5">
                ...
                <a href="{{ route('frontend.packages.index') }}" class="btn btn-orange jatio-bg-color px-4 py-2">
              ...
              <div class="list-group list-group-flush">
                @foreach($groupedRecentReservations->take(3) as $group)
                  @php
                    ...
                    $isMultiPackage = $reservations->count() > 1;
                  @endphp
                  <div class="list-group-item border-0 px-0 py-3 {{ $isMultiPackage ? 'bg-light' : '' }}">
                    <div class="d-flex w-100 justify-content-between align-items-start">
                      <div class="flex-grow-1">
                        @if($isMultiPackage)
                          <h6 class="mb-1 fw-semibold">Multi-Package Booking</h6>
                          <p class="mb-1 text-muted small">{{ $reservations->count() }} packages</p>
                        @else
                          <h6 class="mb-1 fw-semibold">{{ $primaryReservation->packageVariant->package->name ?? 'N/A' }}</h6>
                          <p class="mb-1 text-muted small">{{ $primaryReservation->packageVariant->variant_name ?? 'N/A' }}</p>
                        @endif
                        <small class="text-muted">Status: {{ ucfirst($primaryReservation->booking_status) }}</small>
                      </div>
                      <small class="text-muted ms-3">{{ \Carbon\Carbon::parse($primaryReservation->created_at)->diffForHumans() }}</small>
                    </div>
                  </div>
                @endforeach
              </div>
