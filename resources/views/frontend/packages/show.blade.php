        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="package-detail-card">
                    ...
                    <div class="mb-4">
                        <h4>Select Rider Type</h4>
                        @if ($package->packagePrices->isEmpty())
                            <p class="text-muted">No pricing available for this package.</p>
                        @else
                            @php
                                // Group prices by Rider Type to determine unique options
                                $riderTypes = $package->packagePrices
                                    ->groupBy('rider_type_id')
                                    ->map(function ($prices) {
                                        return $prices->first()->riderType;
                                    });
                            @endphp

                            @foreach ($riderTypes as $riderType)
                                <div class="variant-card" data-rider-type-id="{{ $riderType->id }}">
                                    <div class="row align-items-center">
                                        <div class="col-md-8">
                                            <h5 class="mb-1">{{ $riderType->name }}</h5>
                                            <p class="text-muted mb-2">{{ $riderType->description }}</p>

                                            <div class="price-display">
                                                @php
                                                    // Find prices for this Rider Type
                                                    $typePrices = $package->packagePrices->where(
                                                        'rider_type_id',
                                                        $riderType->id,
                                                    );
                                                    $minPrice = $typePrices->min('price');
                                                    $maxPrice = $typePrices->max('price');
                                                @endphp

                                                @if ($minPrice == $maxPrice)
                                                    <span>TK {{ number_format($minPrice) }}</span>
                                                @else
                                                    <span>TK {{ number_format($minPrice) }} -
                                                        {{ number_format($maxPrice) }}</span>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="col-md-4 text-end">
                                            <button class="btn btn-orange jatio-bg-color select-rider-type-btn"
                                                data-rider-type-id="{{ $riderType->id }}">
                                                Select
                                            </button>
                                        </div>
                                    </div>
            ...
            <div class="col-lg-4">
                <div class="package-detail-card">
                    ...
                    <form id="booking-form" action="{{ route('frontend.cart.add') }}" method="POST">
                        ...
                        <input type="hidden" name="rider_type_id" id="selected-rider-type-id" required>
                        ...
                        <div class="mb-3">
                            ...
                            <div id="time-slots">
                                <div class="loading">
                                    <i class="bi  bi-spinner fa-spin"></i> Select a rider type and date to see slots...
                                </div>
                            </div>
                        </div>
...
@push('scripts')
    <script>
        ...
        // Select Rider Type
        $('.select-rider-type-btn').click(function() {
            const riderTypeId = $(this).data('rider-type-id');
            selectedRiderTypeId = riderTypeId;

            $('.variant-card').removeClass('selected');
            $(this).closest('.variant-card').addClass('selected');

            $('#selected-rider-type-id').val(riderTypeId);
        ...
        function loadTimeSlots() {
            if (!selectedRiderTypeId || !selectedDate) return;

            $('#time-slots').html('<div class="loading"><i class="bi  bi-spinner fa-spin"></i> Loading...</div>');

            $.get('{{ route('frontend.schedule-slots.availability') }}', {
                    package_id: '{{ $package->id }}',
                    rider_type_id: selectedRiderTypeId,
                    date: selectedDate
                })
                .done(function(data) {
                    let slotsHtml = '';
                    if (data && data.length > 0) {
                        data.forEach(function(slot) {
                            const isAvailable = slot.is_open; // API returns is_open based on capacity
                            const slotClass = isAvailable ? 'slot-option' : 'slot-option disabled';
                            const clickHandler = isAvailable ? `onclick="selectSlot(${slot.id})"` : '';
            ...
            <div class="${slotClass}" ${clickHandler}>
              <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="text-end">
                  <div class="price-display">TK ${slot.final_price}</div>
        ...
        function selectSlot(slotId) {
            ...
            if ($('#selected-slot-id').length === 0) {
                $('#booking-form').append('<input type="hidden" name="schedule_slot_id" id="selected-slot-id">');
            }
        ...
        function updateTotalPrice() {
            if (!selectedRiderTypeId || !selectedSlotId) {
        ...
        function updateAddToCartButton() {
            const isValid = selectedRiderTypeId && selectedSlotId && selectedDate && quantity > 0;
        ...
        $('#booking-form').submit(function(e) {
            if (!selectedRiderTypeId || !selectedSlotId) {
                e.preventDefault();
                // Use toastNotifications if available, else alert
                if (typeof toastNotifications !== 'undefined') {
                    toastNotifications.warning('Please select a rider type and time slot.');
                } else {
                    alert('Please select a rider type and time slot.');
                }
                return false;
            }
        });
