...
@section('content')
  ...
  <div class="container my-5">
    ...
    @if($bundlePackages && $bundlePackages->count() > 0)
      <div class="text-center mt-5 mb-3">
        <span class="package-badge mb-3"></span>
        <div class="line-divider"></div>
        <h2 class="bundle-title">Bundle Packages</h2>
      </div>
      ...
      <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4 mb-5">
        @foreach($bundlePackages as $package)
          ...
          @endphp
          <div class="col">
            <div class="custom-card bundle-card" data-package-id="{{ $package->id }}" @if($firstVariant) data-variant-id="{{ $firstVariant->id }}" @endif data-min-participants="{{ $package->min_participants }}" data-max-participants="{{ $package->max_participants }}">
              <img src="{{ $img }}" class="card-img-top" alt="{{ $package->name }}">
              <div class="card-body">
                <h5 class="card-title">{{ $package->name }}</h5>
                <p class="card-text">Bundle Package<br><span class="price-text" data-price-for-variant="{{ $firstVariant->id ?? '' }}" data-display-price="{{ $package->display_starting_price ?? '' }}">Starting from TK {{ $package->display_starting_price ? number_format($package->display_starting_price) : '0' }}</span></p>
                <center>
                  <div class="add-to-cart-container">
                    <a href="#" class="btn jatio-bg-color w-100 btn-add-bundle" @if($firstVariant) data-variant-id="{{ $firstVariant->id }}" @endif>
                    ...
                    <div class="quantity-controls" style="display: none;">
                      <div class="quantity-selector jatio-bg-color w-100">
                        ...
                        <span class="quantity-text">{{ $package->min_participants }} Bundle{{ $package->min_participants > 1 ? 's' : '' }}</span>
    ...
    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
      @forelse($packages as $package)
        ...
        @endphp
        <div class="col">
          <div class="custom-card" data-package-id="{{ $package->id }}" @if($firstVariant) data-variant-id="{{ $firstVariant->id }}" @endif data-min-participants="{{ $package->min_participants }}" data-max-participants="{{ $package->max_participants }}">
            <img src="{{ $img }}" class="card-img-top" alt="{{ $package->name }}">
            <div class="card-body">
              <h5 class="card-title">{{ $package->name }}</h5>
              <p class="card-text">30 (min) Guided Tour<br><span class="price-text" data-price-for-variant="{{ $firstVariant->id ?? '' }}" data-display-price="{{ $package->display_starting_price ?? '' }}">Starting from TK {{ $package->display_starting_price ? number_format($package->display_starting_price) : '0' }}</span></p>
              <center>
                <div class="add-to-cart-container">
                  <a href="#" class="btn jatio-bg-color w-100 btn-add-regular" @if($firstVariant) data-variant-id="{{ $firstVariant->id }}" @endif>
                    <i class="fa-solid fa-cart-shopping me-2"></i> Add to Cart
                  </a>
                  <div class="quantity-controls" style="display: none;">
                    <div class="quantity-selector jatio-bg-color w-100">
                      <button class="quantity-btn quantity-btn-minus" type="button">-</button>
                                              <span class="quantity-text">{{ $package->min_participants }} Package{{ $package->min_participants > 1 ? 's' : '' }}</span>
                      <button class="quantity-btn quantity-btn-plus" type="button">+</button>
                    </div>
                  </div>
                </div>
        ...
        <div class="col">
          <div class="custom-card">
            ...
            <div class="card-body">
              ...
              <center>
                <a href="#" class="btn jatio-bg-color w-100 disabled"><i class="fa-solid fa-cart-shopping me-2"></i> Add to Cart</a>
              </center>
            </div>
          </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function setBtnLoading(btn, isLoading, loadingText) {
    ...
    if (isLoading) {
      ...
      btn.innerHTML = `<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>${loadingText || 'Processing...'}`;
      ...
      fetch(url, { signal: controller.signal })
        .then(async res => {
          ...
          if (!res.ok) {
            ...
            return reject(new Error(`HTTP ${res.status}: ${text || res.statusText}`));
          }
          return res.json().then(resolve).catch(err => reject(new Error('Invalid JSON response')));
        })
        .catch(err => {
          clearTimeout(id);
          reject(err);
  ...
  function bindAddToCart(btn) {
          ...
          .then(slots => {
            ...
            if (open) {
              return fetch(`{{ route('frontend.cart.add') }}`, {
                ...
                body: JSON.stringify([{ variant_id: Number(variantId), quantity: quantity, date: dateStr, slot_id: String(open.id) }])
              })
              .then(async res => {
                let body = null;
                try { 
      ...
      if (currentQuantity > minParticipants) {
        ...
        addToCart(currentQuantity)
          ...
          .catch(err => {
            ...
            currentQuantity++;
            quantityDisplay.textContent = `${currentQuantity} Package${currentQuantity > 1 ? 's' : ''}`;
            toastNotifications.error(`Error updating quantity: ${err.message}`);
          })
          .finally(() => {
    ...
    plusBtn.addEventListener('click', function() {
      if (currentQuantity < maxParticipants) {
        ...
        // Show message when max participants reached
      ...
      addToCart(minParticipants)
        .then(data => {
          if (data && data.success) {
            ...
            toastNotifications.error(data.message || 'Unable to add to cart.');
          }
        })
        .catch(err => {
          toastNotifications.error(`Error adding to cart: ${err.message}`);
        })
        .finally(() => {
          setBtnLoading(btn, false);
        });
  ...
  // Populate prices for visible package variants (both regular and bundle)
      ...
      fetchJson(`/api/pricing/date?date=${today}&variant_id=${variantId}`)
        .then(data => {
          if (data && typeof data.final_price !== 'undefined') {
            span.textContent = 'Starting from TK ' + Number(data.final_price).toLocaleString();
          } else {
            span.textContent = 'Starting from TK —';
          }
        })
        .catch(err => {
          span.textContent = 'Starting from TK —';
  ...
  // Add to Cart: bind all regular buttons (both static and dynamic cards)
  document.querySelectorAll('.btn-add-regular[data-variant-id]').forEach(bindAddToCart);

  // Add to Cart: bind all bundle buttons
  document.querySelectorAll('.btn-add-bundle[data-variant-id]').forEach(bindAddToCart);
