...
@push('styles')
    ...
    <style>
        ...
        .card { border: none; box-shadow: 0 0.125rem 0.375rem rgba(0,0,0,0.1); border-radius: 0.75rem; }
        .card-title { font-weight: 700; margin-bottom: 1.5rem; color: #2c3e50; font-size: 1.25rem; }
        .form-label { font-weight: 600; color: #495057; margin-bottom: 0.5rem; }
        .form-control, .form-select { border-radius: 0.5rem; border: 1px solid #dee2e6; padding: 0.75rem 1rem; transition: all 0.2s ease; }
        .form-control:focus, .form-select:focus { border-color: #F76B19; box-shadow: 0 0 0 0.2rem rgba(247,107,25,0.25); }

        /* ---------- Image Uploader ---------- */
        .image-upload-section { background: #f8f9fa; border-radius: 0.75rem; padding: 1.5rem; margin-bottom: 1.5rem; }
        .badge.jatio-bg-color { background-color: #F76B19; color: #fff; font-weight: 600; }

        /* ---------- Pricing Card ---------- */
        .pricing-card { border: 1px solid #e9ecef; border-radius: 0.75rem; padding: 2rem; background: #ffffff; margin: 1.5rem 0; }
        .pricing-section-title { font-weight: 700; color: #2c3e50; margin-bottom: 1.5rem; font-size: 1.1rem; }
        .pricing-pills { gap: 0.5rem; margin-bottom: 1.5rem; }
        .pricing-pills .nav-link { border-radius: 0.6rem; border: 1px solid #e0e0e0; padding: 0.75rem 1.25rem; font-weight: 600; color: #495057; background: #fff; min-width: 100px; text-align:center; transition: all 0.2s ease; }
        .pricing-pills .nav-link.active { background-color: #F76B19 !important; border-color: #F76B19 !important; color: #fff !important; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(247,107,25,0.3); }
        .pricing-pills .nav-link:hover:not(.active) { background-color: #f8f9fa; border-color: #F76B19; color:#F76B19; transform: translateY(-1px); }

        .price-input .input-group { max-width: 300px; }
        .price-input .input-group-text { font-weight: 600; background: #f8f9fa; border-color: #dee2e6; color: #495057; }
        .price-input .form-control { border-left: none; }
        .price-input .input-group:focus-within .input-group-text { border-color: #F76B19; background: #fffaf7; }

        .selected-day-display { background: #f8f9fa; border-radius: 0.5rem; padding: 1rem; margin-bottom: 1.5rem; border-left: 4px solid #F76B19; }
        .selected-day-display strong { color: #495057; }
        .selected-day-display span { color: #F76B19; font-weight: 600; }

        .btn-save { background: linear-gradient(135deg, #F76B19 0%, #e55e14 100%); color: #fff; font-weight: 600; font-size: 1rem; border-radius: 0.75rem; padding: 1rem 2.5rem; border: none; box-shadow: 0 6px 20px rgba(247,107,25,0.3); transition: all 0.3s ease; position: relative; overflow: hidden; }
        .btn-save:hover { transform: translateY(-3px); box-shadow: 0 12px 25px rgba(247,107,25,0.4); background: linear-gradient(135deg, #e55e14 0%, #d95f17 100%); }
        .btn-save:active { transform: translateY(-1px); }
        .btn-save.btn-loading { pointer-events: none; color: transparent; }
        .btn-save.btn-loading::after { content: ''; position: absolute; width: 20px; height: 20px; top: 50%; left: 50%; margin-left: -10px; margin-top: -10px; border: 2px solid transparent; border-top: 2px solid #ffffff; border-radius: 50%; animation: button-spinner 0.8s linear infinite; }
        @keyframes button-spinner { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }

        .alert { border: none; border-radius: 0.75rem; padding: 1rem 1.5rem; margin-bottom: 1.5rem; }
        .alert-success { background: #d1f2eb; color: #0d5d47; border-left: 4px solid #0d5d47; }
        .alert-danger { background: #fde8e8; color: #c53030; border-left: 4px solid #c53030; }

        .page-header h1 { color: #2c3e50; font-weight: 700; margin-bottom: 0.5rem; }
        .breadcrumb-custom { color: #6c757d; font-size: 0.875rem; margin-bottom: 0; }

        @media (max-width: 768px) { .pricing-card { padding: 1.5rem; } .pricing-pills .nav-link { min-width: 80px; padding: 0.6rem 1rem; font-size:0.875rem; } .btn-save { width: 100%; padding:0.875rem 2rem; } .price-input .input-group { max-width: 100%; } }
        @media (max-width: 576px) { .card-body { padding: 1.25rem; } .pricing-pills { justify-content:center; } .pricing-pills .nav-link { min-width: 70px; padding:0.5rem 0.75rem; font-size:0.8rem; } }

        .is-invalid { border-color:#dc3545; box-shadow: 0 0 0 0.2rem rgba(220,53,69,0.25); }
        .invalid-feedback { font-weight:500; margin-top:0.25rem; }
        .text-muted { font-size:0.875rem; margin-top:0.5rem; display:block; }
        .section-divider { border-top:2px solid #e9ecef; margin:2rem 0; }
    </style>
@endpush
        ...
        <div class="card-body p-4">
            <form id="packageForm" method="POST"
                ...
                <div class="image-upload-section">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <label for="multiple-image-upload" class="form-label">Upload Package Images (Max 4 images)</label>
                            ...
                            <small class="text-muted mt-2">
                                ...
                                Supported formats: JPG, PNG, WebP. Max file size: 5MB. First image will be used as main display.
                            </small>
                        </div>
                    </div>
                </div>
                ...
                <div class="row g-4 mb-4">
                    <div class="col-md-6">
                        ...
                        @error('packageName') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        @error('packageType') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="0.01" class="form-control @error('displayStartingPrice') is-invalid @enderror"
                                name="displayStartingPrice"
                                value="{{ old('displayStartingPrice', $package->display_starting_price ?? '') }}" placeholder="0.00" min="0">
                        </div>
                        @error('displayStartingPrice') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" class="form-control @error('minParticipant') is-invalid @enderror"
                            name="minParticipant"
                            value="{{ old('minParticipant', $package->min_participants ?? 5) }}" min="1" required>
                        @error('minParticipant') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" class="form-control @error('maxParticipant') is-invalid @enderror"
                            name="maxParticipant"
                            value="{{ old('maxParticipant', $package->max_participants ?? 50) }}" min="1" required>
                        @error('maxParticipant') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                </div>
                ...
                <div class="pricing-card">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-lg-6">
                            ...
                            <ul class="nav nav-pills pricing-pills mb-4" id="weekdayPills" role="tablist">
                                @foreach(['sunday','monday','tuesday','wednesday','thursday'] as $day)
                                    @php
                                        ...
                                        $dayPrice = old('weekday_price_'.$day, $package->weekdayPrices[$day] ?? '');
                                    @endphp
                                    <li class="nav-item" role="presentation">
                                        <button type="button"
                                            class="nav-link {{ $day == $oldDay ? 'active' : '' }}"
                            ...
                            <div class="selected-day-display"><strong>Selected Weekday:</strong> <span id="selectedWeekday">{{ ucfirst(old('selected_weekday','monday')) }}</span></div>
                            <div class="price-input">
                                <label class="form-label">Price (৳) <span class="text-danger">*</span></label>
                                <div class="input-group">
                                    <span class="input-group-text">৳</span>
                                    <input type="number" step="0.01" id="weekdayPrice" name="weekdayPrice" class="form-control" value="{{ old('weekdayPrice', $package->weekdayPrices['monday'] ?? 0) }}" placeholder="0.00" min="0" required>
                                </div>
                        ...
                        <div class="col-lg-6">
                            ...
                            <ul class="nav nav-pills pricing-pills mb-4" id="weekendPills" role="tablist">
                                @foreach(['friday','saturday'] as $day)
                                    @php
                                        ...
                                        $dayPrice = old('weekend_price_'.$day, $package->weekendPrices[$day] ?? '');
                                    @endphp
                                    <li class="nav-item" role="presentation">
                                        <button type="button"
                                            class="nav-link {{ $day == $oldDay ? 'active' : '' }}"
                            ...
                            <div class="selected-day-display"><strong>Selected Weekend:</strong> <span id="selectedWeekend">{{ ucfirst(old('selected_weekend','friday')) }}</span></div>
                            <div class="price-input">
                                <label class="form-label">Price (৳) <span class="text-danger">*</span></label>
                                <div class="input-group">
                                    <span class="input-group-text">৳</span>
                                    <input type="number" step="0.01" id="weekendPrice" name="weekendPrice" class="form-control" value="{{ old('weekendPrice', $package->weekendPrices['friday'] ?? 0) }}" placeholder="0.00" min="0" required>
                                </div>
                ...
                {{-- Hidden Fields --}}
                <input type="hidden" id="selected_weekday" name="selected_weekday" value="{{ old('selected_weekday','monday') }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    submitBtn.addEventListener('click', function(e){
        ...
        galleryInput.value = galleryImages.length ? JSON.stringify(galleryImages.map(g=>g.galleryId||g.id).filter(Boolean)) : '';
    ...
    function validateFormClient(){
        ...
        if(parseFloat(weekdayPriceInput.value)<0 || parseFloat(weekendPriceInput.value)<0){
            alert('Prices must be >= 0'); return false;
        }
        return true;
    }
});
