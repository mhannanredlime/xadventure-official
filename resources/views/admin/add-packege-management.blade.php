    ...
    <div class="content-area">
        <div class="container-fluid package-container">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                @can('packages.manage')
                    <div class="dropdown">
                        <button class="btn jatio-bg-color text-white" type="button" data-bs-toggle="dropdown"
                            aria-expanded="false">Add New Package</button>
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="{{ url('/admin/regular-packege-management') }}">Add Regular
                                    Package</a></li>
                            <li><a class="dropdown-item" href="{{ url('/admin/atvutv-packege-management') }}">Add ATV/UTV
                                    Package</a></li>
                        </ul>
                    </div>
                @endcan
            ...
            @php
                $hasAnyRegularPackages =
                    $groupedRegularPackages['Single']->isNotEmpty() ||
            ...
            @else
                <div class="text-center py-4">
                    <i class="bi  bi-box-open fa-3x text-muted mb-3"></i>
            ...
            <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="package-card">
                            @if ($package->display_image_url)
                                <img src="{{ $package->display_image_url }}" class="package-image"
                                    alt="{{ $package->name }}">
                            @elseif($package->image_path)
                                <img src="{{ asset('storage/' . $package->image_path) }}" class="package-image"
                                    alt="{{ $package->name }}">
                            @else
                                <img src="{{ asset('admin/images/pack-2.png') }}" class="package-image"
                                    alt="{{ $package->name }}">
                            @endif
                            <div class="package-details">
                                <h5>{{ $package->name }}</h5>
                                ...
                                @if ($package->variants->isNotEmpty())
                                    ...
                                    @foreach ($package->variants->take(2) as $variant)
                                        ...
                                        @endphp
                                        @if ($weekdayPrice)
                                            <p class="price-item">{{ $variant->variant_name }}: <span>৳
                                                    {{ number_format($weekdayPrice->amount) }}</span></p>
                                        @endif
                                    @endforeach
                                    ...
                                    @foreach ($package->variants->take(2) as $variant)
                                        ...
                                        @if ($weekendPrice)
                                            <p class="price-item">{{ $variant->variant_name }}: <span>৳
                                                    {{ number_format($weekendPrice->amount) }}</span></p>
                                        @endif
                                    @endforeach
                                @else
                ...
                @empty
                    <div class="col-12">
                        <div class="text-center py-4">
                            <i class="bi  bi-motorcycle fa-3x text-muted mb-3"></i>
